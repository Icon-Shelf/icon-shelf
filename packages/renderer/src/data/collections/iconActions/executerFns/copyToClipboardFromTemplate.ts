...
export const copyToClipboardFromTemplate = async ({ icon, actionObj }: ExecuterProps) => {
  const collection = await CollectionsApi.find(icon.collectionId);

  const collectionLoc = collection?.folderSrc || '';
  const relativeIconPath = icon.imageSrc.replace(collectionLoc, '').replace(/^\//, ''); // remove slash at the beginning
